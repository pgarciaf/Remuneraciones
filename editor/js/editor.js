...
(function( $ ){
	...
	var methods = {
		...
		restoreSelection: function(text,mode) {
			//Función para restaurar el rango de selección de texto del editor
			var node;
		...
		restoreIESelection:function() {
			//Función para restaurar el rango de selección de texto del editor en IE
			var range = $(this).data('currentRange');
		    if (range) {
		        if (window.getSelection) {
		            sel = window.getSelection();
		...
		imageWidget: function(){
			//Clase para Widget Manejo de la carga de archivos
			var row = $('<div/>',{
				"class":"row"
			}).append($('<div/>',{
				id :"imgErrMsg"
			...
			handleFileSelect = function(evt) {
				...
				for (var i = 0, f; f = files[i]; i++) {
					//Pasa por todos los archivos
					if(!f.type.match('image.*') || !f.name.match(/(?:gif|jpg|png|jpeg)$/)){ //Solo procesar imágenes
